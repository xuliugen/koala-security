 ...
 */+ function($) {"use strict";
	...
	Tree.prototype = {
		...
		init : function() {
			...
			$.each(self.$element.find('.tree-folder'), function() {
				...
				$folder.on({
					...
					'disSelectCheckBox' : function(e) {
						...
						var treeFolderCheckBox = treeFolder.find('>input');
						if (treeFolderCheckBox.is(':checked')) {
							if (treeFolderContent.find('>.tree-selected').length == 0 && treeFolderContent.find('>.tree-folder>input:checked').length == 0) {
			...
			//添加鼠标右键事件监听
			this.$element.on('contextmenu', '.tree-folder-header,.tree-item', $.proxy(function(ev) {
		...
		createParent : function(value) {
			...
			//如果采用复选框
			if (this.options.useChkBox) {
				$node = $('<div class="tree-folder checkbox" style="display:none;">' + '<input type="checkbox" value="' + value + '">' + '<div class="tree-folder-header">' + '<i class="glyphicon glyphicon-folder-close"></i>' + '<div class="tree-folder-name"></div></div><div class="tree-folder-content"></div>' + '<div class="tree-loader" style="display:none"></div></div>');
			} else {
				$node = $('<div class="tree-folder" style="display:none;">' + '<div class="tree-folder-header"><i class="glyphicon glyphicon-folder-close"></i>' + '<div class="tree-folder-name"></div></div><div class="tree-folder-content"></div>' + '<div class="tree-loader" style="display:none"></div></div>');
			}
		...
		toggleFolder : function(e) {
			...
			var $folder = $el.closest('.tree-folder');
